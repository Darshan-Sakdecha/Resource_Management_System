...
export default async function EditMaintenance({ params }: PageProps) {
    const { id } = await params;

    const maintenance = await prisma.maintenance.findUnique({
        where: { maintenance_id: parseInt(id) },
        include: { resources: true },
    });

    if (!maintenance) return <div className="text-black">Maintenance not found</div>;
    ...
    const scheduledDate =
        maintenance.scheduled_date instanceof Date
            ? maintenance.scheduled_date.toISOString().split("T")[0]
            : "";
    ...
    async function updateMaintenance(formData: FormData) {
        ...
        await prisma.maintenance.update({
            where: { maintenance_id: parseInt(id) },
            data: { resource_id, maintenance_type, status, scheduled_date },
        });
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-black">Edit Maintenance</h1>
                <Link href="/management/dashboard/maintenance" className="bg-gray-700 hover:bg-gray-800 text-white px-4 py-2 rounded">
                    Back
                </Link>
            </div>

            <form action={updateMaintenance} className="bg-white border rounded-lg shadow p-6 space-y-4">
                <div className="grid grid-cols-2 gap-6">
                    <div>
                        <label className="text-black text-sm">Resource</label>
                        <select name="resource_id" defaultValue={maintenance.resource_id} className="border p-2 rounded w-full text-black">
                            {resources.map((r) => (
                    ...
                    <div>
                        ...
                        <select name="maintenance_type" defaultValue={maintenance.maintenance_type} className="border p-2 rounded w-full text-black">
                    ...
                    <div>
                        ...
                        <select name="status" defaultValue={maintenance.status} className="border p-2 rounded w-full text-black">
                    ...
                    <div>
                        ...
                        <input type="date" name="scheduled_date" defaultValue={scheduledDate} className="border p-2 rounded w-full text-black" />
                    </div>
                </div>
